...
## Returns the parsed VDF data in Dictionary form
...
## Attempts to parse the vdf string data provided. 
## Returns an [Error]. If the parse was successful, it returns OK and the result can be retrieved 
## using data. If unsuccessful, use [method get_error_line] and [method get_error_message] for identifying 
