...
use crate::dbus::udisks2::block::{BlockProxy, BlockProxyBlocking};
    ...
    pub fn from_path(path: GString) -> Gd<Self> {
        ...
        RUNTIME.spawn(async move {
            if let Err(e) = run(tx, dbus_path).await {
                godot_error!("Failed to run BlockDevice task: ${e:?}");
            }
        });
...
async fn run(tx: Sender<Signal>, path: String) -> Result<(), RunError> {
    ...
    let proxy = BlockProxy::builder(&conn).path(path)?.build().await?;
