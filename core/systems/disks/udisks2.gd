...
var logger := Log.get_logger("UDisks2", Log.LEVEL.DEBUG)
...
func get_devices() -> Array[BlockDevice]:
	...
	#logger.trace("Searching for " + pattern + " objects.")
	...
	for obj in device_paths:
		...
		if path.contains("block_devices"):
			#logger.trace("Found " + pattern + " in " + path)
...
func _drives_updated(iface: String) -> void:
	if not iface.contains(DRIVES_PATH):
		return
	print("Interface Updated: " + iface)
