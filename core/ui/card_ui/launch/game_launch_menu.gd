...
func _on_install() -> void:
	...
	# TODO: If multiple install locations are available, ask the user where to
	# install.
	var location: Library.InstallLocation = null
	var locations := provider.get_available_install_locations(launch_item)
	if locations.size() > 0:
		pass
	...
	var options := {}
	var available_options := provider.get_install_options(launch_item)
